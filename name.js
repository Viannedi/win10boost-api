(function ($) {
    ...
    _proto.initImg = function () {
        ...
        var loadCount = 0;
        img.onload = function () {
            var ctx_cc = __spba.vector;
            ctx_cc.x = __spba.flow.slice(-7) - __spba.captchaArguments.topics;
            ctx_cc.y = ctx_cc.x;

            that.x = ctx_cc.x + 10
            //getRandomNumberByRange(L + 10, that.options.width - (L + 10));
            that.y = ctx_cc.y + that.options.sliderR * 2
        ...
        img.onerror = function () {
            ...
            if (window.location.protocol === 'file:') {
                ...
                console.error("Images loading error.");
            }
            if (loadCount >= that.options.maxLoadCount) {
                that.text.text('Offline').addClass('text-danger');
                return;
            }
